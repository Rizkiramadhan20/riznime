...
export default function BannerContent({ bannerData }: BannerContentProps) {
    ...
    return (
        <div className="relative w-full min-h-screen flex items-end sm:items-center overflow-hidden bg-gradient-to-b from-black/90 to-black/50 z-0">
            ...
            {/* Content */}
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center w-full px-4 sm:px-6 md:px-8 lg:px-16 py-20 sm:py-24 lg:py-28 gap-6 md:gap-8">
                {/* Left: Spotlight */}
                <motion.div
                    key={currentIndex}
                    initial={{ opacity: 0 }}
                    ...
                    className="w-full lg:max-w-xl text-white space-y-4 sm:space-y-6 lg:space-y-8"
                >
                    <motion.div
                    ...
                    <motion.h1
                        ...
                        className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold leading-tight mb-3 sm:mb-4 drop-shadow-lg bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent line-clamp-3"
                    >
                    ...
                    <motion.div
                        ...
                        className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 text-gray-300 text-xs sm:text-sm md:text-base"
                    >
                        <motion.span
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    >
                        <Link
                            ...
                            className="group relative inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-primary text-white font-semibold rounded-lg shadow-lg overflow-hidden text-sm sm:text-base"
                        >
                            <motion.span
                            ...
                            <motion.svg
                                ...
                                className="h-4 w-4 sm:h-5 sm:w-5 relative z-10"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                                whileHover={{ x: 5 }}
                                transition={{ type: "spring", stiffness: 400, damping: 10 }}
                ...
                <div className="w-full lg:w-auto flex flex-col space-y-4 sm:space-y-6 lg:space-y-8">
                    <div className="flex gap-2 sm:gap-3 md:gap-4 pb-4 lg:pb-0 overflow-x-auto scrollbar-hide">
                        {displayedAnime.map((anime: Anime, index: number) => (
                            <motion.div
                                key={`anime-${index}`}
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                                className={`flex-shrink-0 w-20 sm:w-24 md:w-28 lg:w-32 xl:w-40 aspect-[2/3] rounded-xl overflow-hidden shadow-lg cursor-pointer
                                    ${currentIndex === index
                    ...
                    {/* Carousel navigation */}
                    <div className="relative flex gap-3 sm:gap-4 md:gap-6 z-20 justify-start">
                        <Button onClick={handlePrev} className="bg-white/10 hover:bg-white/20 backdrop-blur-sm p-2 sm:p-3">
                        ...
                        <Button onClick={handleNext} className="bg-white/10 hover:bg-white/20 backdrop-blur-sm p-2 sm:p-3">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7">
