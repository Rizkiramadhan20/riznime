...
import { BookmarkItem } from '@/utils/context/types/Auth'
...
export default function BookmarksLayout() {
    const { user } = useAuth()
    const { bookmarks, loading, error } = useBookmarks(user?.uid)
    const [search, setSearch] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 9;

    const filteredBookmarks = bookmarks.filter((item: BookmarkItem) =>
    ...
    return (
        <section className="min-h-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {paginatedBookmarks.map((item: BookmarkItem) => (
                    <Link href={item.href} key={item.animeId} className="bg-[var(--card-bg)] rounded-xl shadow-md p-4 flex flex-col gap-2 border border-[var(--border-color)] group">
                        ...
                        <div className="text-xs text-gray-500">Disimpan pada: {new Date(item.addedAt).toLocaleString()}</div>
                    </Link>
                ))}
            </div>
            ...
            {filteredBookmarks.length === 0 && (
                ...
                >
                    <p className="text-sm sm:text-base">Belum ada bookmark yang tersimpan</p>
                </motion.div>
            )}
        </section>
    )
