...
async function getAnimeSlugs(): Promise<Array<{ slug: string; updatedAt: Date }>> {
    try {
        ...
        try {
            ...
            } else if (ongoingData && (ongoingData as OngoingAnimeData).ongoing && (ongoingData as OngoingAnimeData).ongoing.animeList) {
                // Handle the case where data has nested structure
                (ongoingData as OngoingAnimeData).ongoing.animeList.forEach((anime: Anime) => {
                    if (anime.href) {
                        slugs.push({
                            slug: anime.href,
                            updatedAt: new Date()
...
async function getMangaSlugs(): Promise<Array<{ slug: string; updatedAt: Date }>> {
    try {
...
async function getDonghuaSlugs(): Promise<Array<{ slug: string; updatedAt: Date }>> {
    try {
